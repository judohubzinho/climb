...
import gruposSvg from "../../imports/17_-_TELA_NETWORKING_-_GRUPOS__1_.svg";
import NavDropdown from "../components/NavDropdown";
import AuthNavSlot from "../components/AuthNavSlot";

/* SVG canvas: 1900 × 4206 */
const W = 1900;
const H = 4206;
...
export default function NetworkingGrupos() {
  ...
  return (
    <div style={{ position: "relative", width: "100%", background: "#080808" }}>
      <img src={gruposSvg} alt="Networking — Grupos — CLIMB"
      ...
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
        ...
        <button aria-label="Acessibilidade em Libras"
          style={{ position: "absolute", left: x(1765), top: y(1580), width: w(80), height: h(80), pointerEvents: "auto", background: "transparent", border: "none", cursor: "pointer", borderRadius: "50%" }} />

        {/* ── Busca ── */}
        <div style={{ position: "absolute", left: x(395), top: y(690), width: w(1105), height: h(80), pointerEvents: "auto" }}>
          <input type="text" value={busca} onChange={e => setBusca(e.target.value)} style={searchInput} aria-label="Buscar grupos" />
        </div>

        {/* ── "Entrar no grupo" — 6 grupos (levam pro Cadastro) ── */}
        {[1875, 2800].map(ry => CARD_X.map((cx, i) => (
          <button key={`${ry}-${i}`} type="button" onClick={() => navigate("/cadastro")} aria-label="Entrar no grupo"
            style={{ position: "absolute", left: x(cx), top: y(ry), width: w(CARD_W[i]), height: h(55), pointerEvents: "auto", background: "transparent", border: "none", cursor: "pointer", borderRadius: 999 }} />
        )))}

        {/* ── Footer logo → home ── */}
        <Link to="/" style={{ position: "absolute", left: x(395), top: y(3544), width: w(140), height: h(50), pointerEvents: "auto", display: "block" }} aria-label="CLIMB footer" />
      </div>
    </div>
  );
}

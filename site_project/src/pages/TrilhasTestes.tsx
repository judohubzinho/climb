...
import trilhasSvg from "../../imports/09_-_TELA_CURSOS_-_TRILHAS_E_TESTES__1_.svg";
import NavDropdown from "../components/NavDropdown";

/*
  SVG canvas: 1900 × 3987
*/
const W = 1900;
const H = 3987;
...
export default function TrilhasTestes() {
    ...
    <div style={{ position: "relative", width: "100%", background: "#080808" }}>
      ...
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
        ...
        <button aria-label="Acessibilidade em Libras"
          style={{ position: "absolute", left: x(1765), top: y(1565), width: w(80), height: h(80),
            pointerEvents: "auto", background: "transparent", border: "none", cursor: "pointer",
            borderRadius: "50%" }} />

        {/* ── Busca ── */}
        <div style={{ position: "absolute", left: x(390), top: y(725), width: w(1140), height: h(90),
          pointerEvents: "auto" }}>
          <input type="text" value={busca} onChange={e => setBusca(e.target.value)}
            style={searchInput} aria-label="Buscar trilhas ou testes" />
        </div>

        {/* ── "Fazer agora" — trilhas de aprendizado (levam pro Cadastro) ── */}
        {CARD_X.map((cx, i) => (
          <button key={`trilha-${i}`} type="button" onClick={() => navigate("/cadastro")}
            aria-label="Fazer trilha agora"
            style={{ position: "absolute", left: x(cx), top: y(1840), width: w(CARD_W[i]), height: h(75),
              pointerEvents: "auto", background: "transparent", border: "none", cursor: "pointer",
              borderRadius: 999 }} />
        ))}

        {/* ── "Fazer agora" — testes interativos (levam pro Cadastro) ── */}
        {CARD_X.map((cx, i) => (
          <button key={`teste-${i}`} type="button" onClick={() => navigate("/cadastro")}
            aria-label="Fazer teste agora"
            style={{ position: "absolute", left: x(cx), top: y(2925), width: w(CARD_W[i]), height: h(75),
              pointerEvents: "auto", background: "transparent", border: "none", cursor: "pointer",
              borderRadius: 999 }} />
        ))}
        ...
        {/* ── Footer logo → home ── */}
        <Link to="/"
          style={{ position: "absolute", left: x(405), top: y(3325), width: w(140), height: h(50),
            pointerEvents: "auto", display: "block" }}
